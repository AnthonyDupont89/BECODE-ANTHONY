...
function App() {
  ...
  const validateField = ((name, value) => {
    ...
    if(name === "password2" && value !== formData.password1) {
      setErrors((prev) => {        
        return {...prev, [name]: "*Password verification must be the same as password" };
      });
    }
  });
  ...
  const handleSubmit = ((e) => {
    ...
    const completedForm = formDataValues.every(value => value);
    const anyErrors = errorsValues.every(value => !value);

    if(completedForm && anyErrors) {
